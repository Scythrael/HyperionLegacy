...
- **⭐⭐ THE UNSCANNABLE VESSEL: an easter-egg rare encounter that FORESHADOWS The First Cause (user 2026-09-02, concept stage, implementation undecided).** The scenario in the user's words: you are grinding pirate patrols over and over, and either on screen or LATER IN THE COMPLETED-EVENTS LOG you see something about a rare encounter that utterly destroyed you. When you meet it in person you stand no chance. **A mysterious, unrecognized vessel that cannot be scanned. To your sensors it does not exist.**
  ...
  - ⚠️ **PEACE TEST (raised by Claude, must be settled before building):** "utterly destroyed without a chance" as a PERMANENT hull + installed-gear loss is an unpreparable punishing random event, which is exactly what [[feedback_build_ease_of_use_sell_peace]] rejects. **RECOMMENDATION: it dismisses you, it does not kill you.** Reuse the combat engine's EXISTING `limpingHome` defeat state so the ship returns damaged and needs repair: the player loses the RUN, not the fleet. This is also better horror. Surviving to find that your sensors recorded NOTHING is creepier than a death screen; the dread is the absence, not the loss.
...
- **⭐⭐ THE DBD ESCALATION ARC: the unscannable vessel becomes a progression system (user 2026-09-02).** Extends the easter-egg entry above into a full long-arc design, and WELDS TOGETHER three previously separate ideas: the unscannable vessel, rare-encounters-as-an-exclusive-component-source, and the [[project_fleet_admiral_first_cause_dt]] north-star. The mystery does not sit beside the progression, it BECOMES the progression.
  - **The arc (user's words, structured):** the encounter is **very, very, very rare at the start**, and its frequency is **gated by CAMPAIGN CHAPTER COMPLETION**, rising gradually as the DBDs (Das Bad Doods, the First Cause's dispatched agents per the lore doc) take a more direct interest in Humanity. **Eventually these ships stop being unstoppable behemoths**: late-arc they become a rare encounter the player can actually BEAT, and they **drop some of their tech**.
  ...
  - **Peace test still applies** (see the entry above): the early unwinnable version should DISMISS rather than destroy (reuse `limpingHome`), so the player loses the run and not the fleet. The late fightable version is a normal combat encounter and needs no special handling.
