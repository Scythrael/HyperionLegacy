...
- **0.13.x ROADMAP = a TAB-BY-TAB ease-of-use sweep of the console, one perspective per point release (user 2026-09-01).** The organizing spine for the whole series: each 0.13.x cleans up one console perspective (the design-system-accretion / UI-readiness discipline made concrete as a cadence). Confirmed sequence:
  ...
  - **0.13.5 = OPTIONS / SYSTEM menu overhaul.** The options menu is currently a bit of a mess; reorganize it: proper SUBTABS, replace the little color-blot theme swatches with real DROPDOWNS (select) for theme + other settings, general grouping/cleanup. ⚠️ INCLUDES the DEV PANEL: 0.13.5 gets the dev panel ORGANIZED + PREPARED to be spun out into its own dedicated system in 0.14.0 (ties to the dev/analytics + legitimacy/anti-cheat groundwork in [[project_fleet_admiral_online_strategy]]; the dev panel becoming "its own beast" is a 0.14.0 thing, 0.13.5 just readies it).
