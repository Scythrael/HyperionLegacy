...
- **✅ BUILT 2026-09-04 (Salvage Lanes). SALVAGE LANES: parallel salvage slots bought on a Salvage Bay Upgrades tab (user, 2026-09-04, originally "for later").** The player's framing: queue 5000 of an item on one lane and get one per minute; buy a second lane and the same queue yields two per minute, because the lanes run concurrently. Throughput, bought with an upgrade track, on top of the existing queue depth.
  - **WHAT SHIPPED.** `FACILITIES.salvageBay` (a two-rung lane track, credits + Fleet-Admiral-level gates, no materials and no talent gate), `SALVAGE_BAY_BASE_SLOTS = 1` plus the derived `salvageSlotCount(state)` replacing the flat `SALVAGE_SLOT_COUNT` constant, an Upgrades sub-tab on the bay's console reusing the shared `facilityUpgradeButton` / `canBuildFacilityUpgrade` / `startFacilityUpgrade` machinery, and `SAVE_VERSION` 42 with an additive `MIGRATIONS[41]`. Lane counts: level 0 = 1, level 1 = 2, level 2 = 3 (the same ceiling the Refinery and Fabricator reach).
  - **THE HAZARD, HANDLED: level 0 IS a working one-lane bay.** The bay had no level track, so every existing save sits at level 0 and many hold queued and in-flight salvage. The track therefore has NO founding rung and the lane helper is base-plus-rungs rather than sum-from-zero, so a level-0 bay is operational (the Fuel Depot / Shipyard-bay-floor posture, not the Refinery / Shipyard founding posture). Pinned by a `save.test.ts` block that migrates a genuine pre-upgrade save, salvage job in flight and order queued, and proves the bay still runs.
  - **STILL OPEN: a salvage SPEED rung, deliberately deferred.** The Upgrades tab sells lanes and only lanes. Salvage durations were retuned the same day (base job 60 ticks), and a second throughput lever landing on the same track in the same pass would make a bad feel impossible to attribute to either knob. Speed wants its own design pass against the retuned durations. The track is finite and extends additively (the Refinery's mixed slot+speed track is the precedent), so adding a `{ salvageSpeedMult }` rung later is a data change plus one derive-on-read helper, mirroring `refineSpeedMult`.
  - **ALSO OPEN, cosmetic:** the Facilities roster card for the Salvage Bay still shows the subtitle "Recycling bay" where every other levelled facility shows "Level N". The swap was held back only because the build ran under a zero-visible-string-removals gate; the level is printed on the bay's own Upgrades tab. One-line change whenever the cards should be uniform.
  - **ORIGINAL ENTRY BELOW, kept for the reasoning that drove the build.**
...
- **⭐ SHIPYARD BERTHS: explicit construction + repair berth upgrades (user, 2026-09-04, "for later"). ⚠️ READ THIS FIRST: MOST OF IT ALREADY EXISTS.** The user described construction berths that can build OR repair, repair berths that can only repair, a repair queue when nothing is free, and a free construction berth taking a waiting repair when no build is queued. **`processShipRepairs` in `tick.ts` already implements that behavior**, so do not rebuild it. Verified 2026-09-04:
  ...
  - **WHAT IS ACTUALLY MISSING**, and all this entry should cover:
    1. ✅ **DONE (Shipyard Berths, 2026-09-06). Berths are now an explicit upgrade.** `FACILITIES.shipyard` gained two `{ addShipyardBays: 1 }` rungs (levels 3→4 and 4→5, 50k/125k credits, FA 11/14), and `shipyardBayCount` sums them ON TOP of the existing `+1 per build-speed rung` grant. The old grant was deliberately KEPT rather than re-homed, because bays are derived on read, so moving it would hand existing saves fewer bays than they had; `ship-repair.test.ts` holds a never-fewer-bays proof asserting the live count >= the pre-berths count at every reachable level. No SAVE_VERSION bump was needed. The console now NAMES the bay on the speed rungs too, which was the actual complaint.
    2. ✅ **DONE (`6001684`). The ship build QUEUE shipped**, with a slots readout that deliberately counts BUILD capacity (`shipBuildSlotCount`, bays minus one) rather than total bays.
    3. **No dedicated repair-only track.** ⚠️ **STILL OPEN, AND DELIBERATELY OUT OF SCOPE of the 2026-09-06 berths work** (user scope, verbatim: "Not the repair/construction options, but multiple lanes, and queuing"). Bays remain ONE SHARED POOL. Read the design question below before building this.
...
- **⭐⭐ THE STARBASE: Docks retired and replaced, with docking bays folded into its upgrade track (user decision, 2026-09-04).** Docks as a facility GO AWAY. The Starbase replaces them and gives the capability a proper home with room to grow. The existing capacity keeps working, renamed in place.
...
- **⭐ CAMPAIGN FLAGS: campaign progress as a game-state gate (user, 2026-09-04, "needs to be logged").** Campaigns will be a **flagged game state** that enables and disables features. Reaching campaign X turns things on: the stated example is that random enemy invasion attempts on the Starbase become possible only past a certain campaign point.
  - Worth building as real infrastructure rather than scattered booleans: one campaign-progress value in the save plus a single predicate other systems ask ("is this unlocked yet"), so a feature gate is one call and never a copy-pasted level check. The existing FA-level and talent gates are the shape to follow, and the exhaustive-`Record`-over-a-union trick applies again if campaigns become an enumerated list.
  - Interacts with a lot later (invasions, drop tables, difficulty, the mystery-component loop's source tiers), so the seam is worth having early even while the content is thin.

- **⭐ TWO KINDS OF DOCKING SLOT: long-term berths versus turnaround slots (user, 2026-09-04; wanted for 0.13.4's mission patch, the prospecting linkage kept ON ICE).** Two distinct slot types on the Starbase, mirroring how the Shipyard already splits build capacity from repair capacity:
    1. **Drydock berths** for ships that simply PARK there (today's docks capacity).
    2. **Turnaround slots** for ships "just passing through": a ship returning from a mission needs one FREE to dock and unload its cargo. Name not settled; the user dislikes their own placeholder ("visitation ports"). Candidates that fit by definition: **Transit Berth**, **Turnaround Bay**, **Cargo Gantry**, **Unloading Gantry**. Airport parlance is the right instinct: a gate (short, high turnover) versus a hangar (long stay).
  - **Behaviour:** 10 turnaround slots and 10 ships means nobody ever waits. 4 slots and 10 ships means ships queue to dock and unload on a schedule, adding transit, docking and unloading phases with a visible "waiting to unload" status. The user's framing: "it extends the time taken for your missions, but that is just logistics." Another upgrade track, more to do.
  - **PLANNED SHAPE:** land it in **0.13.4 (the missions patch)**, adding the slots with all the hooks the missions system needs to implement against.
  - ⚠️ **THIS IS A FRICTION MECHANIC, and the standing value is "build ease of use, sell peace, never build in stress / friction / punishment".** Not a veto: a visible capacity constraint with a purchasable fix is good idle design and is the same shape as docks capacity. But the line between satisfying logistics and "why is my mission slow" is **visibility and agency**. Requirements, not suggestions: the wait must be a NAMED, VISIBLE status ("Waiting to unload, 3rd in line") carrying queue position and ETA, the cause must be legible at a glance, and the fix must be reachable and affordable. A silent time extension reads as the game being slow, not as a system the player is managing.
  - ⚠️ **SOFTLOCK RISK, with direct precedent.** 0.11.1 shipped as an EMERGENCY fix because docks filled up with no in-game way to remove a ship, so acquiring one more bricked the save. "You cannot unload without a free turnaround slot" has exactly that shape. Design the escape valve BEFORE building: what happens when every slot is occupied by something that cannot clear? Cargo must never be permanently strandable and a player must never be unable to act. Candidates: a slot a player action can always free, an overflow path that unloads slowly rather than not at all, or a guaranteed-free slot exactly as `shipBuildSlotCount` reserves the last bay for repair. The Shipyard already solves this class of problem in-repo and is the precedent to copy.
  - **ON ICE, explicitly:** tying turnaround slots specifically to PROSPECTING missions. The user likes it but is not certain, so do not assume it.

- **⭐ FUEL TYPES: selectable fuel recipes, and the reason the Fuel Depot has no queue (user decisions, 2026-09-06).** Two linked decisions, recorded together because the second is CONTINGENT on the first and reading either alone gives the wrong answer.
  - **DECIDED, and shipped in 0.13.3: the Fuel Depot gets NO QUEUE.** Unlike every other production facility, `processFuelPipelines` is always-on and fully automatic: every tick it fills every free pipeline from the ONE fuel recipe while there is tank room and ice. There is no order object, no manual start and no recipe choice, so **there is no player intent for a queue to carry**. Worse, promotion runs before the automatic pass on the same tick, so a promoted batch would claim a pipeline the automatic pass was about to fill with an identical batch: the resulting state is byte-identical except the player's queue entry was silently spent. It ships as a triple-braked documented stub (no free slot, a `notImplemented` reason, and a shape refusal), so the DECISION is recorded and type-checked rather than a control existing that does nothing.
  - **SELECTABLE FUEL TYPES ARE COMING, AND THEY DO NOT RE-OPEN THE QUEUE (user, 2026-09-06).** The decisive clarification, in the user's words: **when you select a specific type it just auto-refines like it does now.** So a fuel type is a persistent **SETTING**, not an order. The automatic pass keeps running and simply reads a different recipe. A setting is not something a queue can hold, because there is no discrete unit of work to line up, so the stub stays correct even once fuel is multi-type. That is a STRONGER position than "correct for one recipe": the automatic model is what makes a queue meaningless, and the automatic model is being kept deliberately, because set-and-forget is the point.
  - **The ONE thing that would re-open it:** if players ever needed to SEQUENCE types ("make 1000 of A, then switch to B"), that is a queue of discrete orders and the argument stops holding. Nothing in the current direction asks for that. Do not build toward it speculatively.
  - **So the actual work for multi-type fuel is a type SELECTOR plus a saved selection, NOT a production-line conversion.** Much smaller than it first appears: the pipelines, their auto-fill behaviour and the four existing upgrade branches all stay exactly as they are, and only the recipe they read changes.
  - **The real design fork to settle first, before any of the above:** fuel is currently a single `Decimal` currency in one capped tank. Multiple fuel types could mean (a) separate tanks per type, which multiplies the capacity UI, the fuel-runway projection and the auto-buy fallback by the number of types; (b) one tank holding a blend, which makes "what is in my tank" ambiguous and complicates the runway; or (c) one tank plus a quality or efficiency attribute on the fuel, so a better fuel yields more range per unit rather than being a separate resource. **(c) is the cheapest to reason about and the least disruptive to the existing runway maths**, but it is genuinely a design choice with different gameplay, not a technicality. ⚠️ Whichever is chosen, the fuel runway is LOCKED as measured-EMA plus a pure two-phase projection, and any multi-type model has to keep that honest or explicitly re-open the lock.
  - Also interacts with: the auto-buy credit fallback (which fuel does it buy?), mission fuel costs (do missions require a specific type?), and the Fuel Depot's existing four upgrade branches, whose yield and intake multipliers currently assume one recipe.

- **⭐⭐ THE LANE ALLOCATION MODEL: free lanes join a running order once nothing new is waiting (user decision, 2026-09-10; NOT for 0.13.3).** The production model for EVERY queueing facility, crafting and salvage alike. This is a rework of how work maps onto lanes, not a UI change, and it is parity-critical.
  - **THE RULE, in one sentence:** a free lane takes the next **unstarted** queued order first; if no unstarted order is waiting, it **joins an order already running** and pulls one unit at a time from its remaining pool.
  - **The user's own worked example, which is the spec:** queue 1000 of item A, 1000 of item B and 1000 of item C at a two-lane facility. A and B start on the two lanes; C waits. A finishes first (it is faster), so that lane starts chewing C one unit at a time. Later B finishes, and that lane starts chewing C as well. Both lanes now work C concurrently.
  - **WHY THIS AND NOT EITHER PURE MODEL.** Two models were compared and both were rejected alone:
    - **Model A (what craft lines do today):** a batch of N is bound to ONE lane and runs its iterations there; other lanes need their own orders. More lanes buys **more different things at once**, but a lane sits idle whenever you have not configured enough separate orders, and buying a lane does nothing for work already queued.
    - **Model B (what batch salvage does today):** a batch is a POOL and any free lane pulls from it. More lanes buys **the same order finishing faster**, which makes a lane purchase feel immediately worth it, but one big batch MONOPOLISES the facility: queue 5000 ingots and your polysilicate order waits hours behind it.
    - **The chosen middle path gets both payoffs**: one order alone uses every lane you own (B's payoff), while two distinct orders each get a lane and run concurrently (A's payoff). No lane idles while work exists, and no distinct order starves behind a big batch.
  - ⚠️ **THE REAL ARCHITECTURAL CHANGE: orders stop owning lanes.** Today a craft line IS a lane, with `line.remaining` living on the lane itself, which is exactly why a batch cannot spread. This model separates THE WORK from THE MACHINE: an order carries its own remaining count and holds a **lane allocation that changes over its life** (C has 0 lanes, then 1, then 2). The user's phrasing "1000 of A in lane A" is the natural way to say it but is not the model: lanes are interchangeable capacity, not homes.
  - ⚠️ **PARITY IS THE GATE, and it is a harder gate than usual.** This changes WHAT RUNS ON WHICH TICK, so offline-equals-live must be re-proven from scratch rather than assumed to carry: which lane joins which order, in what order units are pulled, and the rng draw order across several lanes drawing from one pool on the same tick. The 0.13.3 multi-lane salvage parity cases (several jobs completing on one tick, three consumers of one stack) are the shape to extend, not to trust as-is.
  - **THREE THINGS TO DECIDE BEFORE BUILDING:**
    1. **Which running order does a freed lane join** when several still have units? Oldest-first matches the FIFO used everywhere else and is the default recommendation, but soonest-to-finish and spread-evenly both have arguments and produce visibly different play.
    2. **Is there a per-order lane cap?** If one order is queued and six lanes are owned, does it take all six? Probably yes, but that is also precisely how one order eats a facility for an hour, so it should be a deliberate choice rather than a fallout.
    3. **The ETA readout must handle allocation changing MID-FLIGHT.** An order reading "1000 left, about 16 hours" will suddenly halve when it picks up a second lane. That is a GOOD moment, the payoff for buying a bay made visible, but only if the number updates cleanly instead of looking broken. Design the readout for a changing rate rather than a fixed one.
  - **NOT FOR 0.13.3, deliberately.** That release is built, gated and mid-QA, and this is a production-model change to its headline system. It wants its own release and its own parity work. Candidate for 0.13.4 or later.
  - Applies to **all** queueing facilities: Refinery, Fabricator, Research Lab, Salvage Bay, Shipyard. Not the Fuel Depot, whose pipelines stay automatic (see the FUEL TYPES entry). Note the Shipyard interaction: `BUILD_CONCURRENCY_CAP` is 1 today, so until that is revisited the Shipyard has one build lane regardless and this model changes nothing there.

- **⭐ PER-FACILITY QUEUE-DEPTH TALENTS: separate points per facility instead of one global grant (user, 2026-09-10; NOT for 0.13.3).** Today the `fleetLogisticsQueue1/2/3` chain grants queue depth GLOBALLY: one number from `queueDepth(state)` applied to every queueing facility at once. The user wants depth bought **per facility**, so points spent on the Refinery's queue do not deepen the Fabricator's.
  - **This REFINES the LOCKED section 15 pattern rather than contradicting it.** Section 15 already says depth is per facility in the sense that each facility holds its own queue of `depth` entries. What changes is that the NUMBER becomes per facility too, instead of one global value applied everywhere. Lanes and depth stay independent axes, and the no-depth-per-lane rule is untouched.
  - **The signature change is small and the compiler will find the callers:** `queueDepth(state)` becomes `queueDepth(state, facility)`. The talent effect payload gains a facility key, or the union gains one member per facility.
  - **Why it is worth doing, in the user's own established reasoning:** it is more to buy and more to plan without being extension for its own sake. It also creates a genuine early-game CHOICE, since a player with few points must deepen the facility they actually work at rather than raising everything shallowly, and that choice changes as their play changes.
  - ⚠️ **THE COST TO WATCH IS TALENT-WEB LEGIBILITY.** Three nodes become roughly five facilities times however many rungs, so a 3-node chain could become 15. The talent web is a radial pan-around UI; adding a dozen near-identical nodes risks turning a readable tree into a wall of the same icon. Consider one node per facility with repeatable ranks, or a single node whose rank is allocated per facility, rather than a flat 15 nodes. **Decide the SHAPE before the numbers.**
  - ✅ **THE SHAPE IS DECIDED (user, 2026-09-10): ONE SHARED TRUNK, THEN PER-FACILITY BRANCHES.** A single global first point takes the default queue from 1 slot to 2 **at every facility** ("double the efficiency of the default queuing system"), and per-facility slots branch off from there. This answers the legibility problem above: the first purchase is one node with a broad, immediately felt benefit, and specialisation only begins once a player knows which facility they actually live at. It also gives the early game a genuinely useful cheap buy instead of forcing a specialisation choice before the player has the information to make it.
  - ⚠️ **KEEP THE EFFECT ADDITIVE EVEN THOUGH THE FLAVOUR SAYS "DOUBLE".** `queueDepth` sums additive `{ type: "queueDepth" }` payloads, and at the base of 1 a `+1` IS a doubling, so the player-facing wording stays true. If the effect were literally a MULTIPLIER, ordering would start to matter the moment per-facility points exist: double-then-add yields 3 while add-then-double yields 4, and the answer would be decided by evaluation order rather than by design. Additive effect plus doubling language gets the intended feel with none of the ambiguity. The same reasoning applies to any later "double it again" rung: express it as the integer it resolves to.
  - **Consequence for the existing chain:** `fleetLogisticsQueue1` becomes (or is replaced by) the shared trunk, and `fleetLogisticsQueue2/3` are the natural candidates to become per-facility branches. That still has to honour points a player has ALREADY spent on 2 and 3 without silently losing them, which is the migration question left open above.
  - **Respec already handles the downgrade path safely**, which removes the main risk of letting players commit points to the wrong facility: dropping depth below the number of queued entries lets the over-cap entries DRAIN rather than truncating them (the shipped over-cap precedent). Verify that still holds per facility.
  - Open: whether the existing global chain is migrated into per-facility grants (and how a player's already-spent points are honoured, which must not silently lose them), or whether the global chain stays as a baseline with per-facility nodes on top. The first is cleaner; the second avoids a talent migration entirely.

- **⭐ CONTEXTUAL HELP: a square `?` button per screen, deep-linking into the existing manual (user, 2026-09-10).** The user could not find the auto-salvage rules on their own game (they sit on the Salvage Bay's Rules tab, above the scroll position on a phone) and proposed a fix: a square `?` button you tap to get the help for what everything on that screen means. Raised alongside a related idea from the same conversation, that tapping the SCREEN NAME should show the facility's description as a tooltip.
  - **The two ideas converge; prefer the BUTTON.** A `?` is a **visible affordance**; a tappable heading is invisible and nobody discovers it. Build the button; the tap-the-title behaviour can ride along as a bonus but must never be the only route.
  - ⭐ **THE KEY ARCHITECTURAL POINT: the content ALREADY EXISTS, so the button must LINK, never duplicate.** `src/lib/helpTopics.ts` gained **Order Queues**, **Crafting Levels** and **Auto-Salvage** topics in 0.13.3, plus corrected `salvage` and `fabricating` topics. A `?` button should deep-link to the relevant topic id, not carry its own prose. Two copies of an explanation is two things to keep true, and the second one is the one that silently goes stale. There is already a `helpTopics.test.ts` enforcing house rules (including the INSTALL vocabulary), so linked content inherits that guard for free, while inline copy would not.
  - **Suggested shape:** a per-screen map from console/sub-tab to a help topic id, ideally a total record over the console key union so a new screen without help is a COMPILE ERROR rather than a silently missing button. Same trick as `QUEUE_ADAPTERS` and the icon registry recommendation.
  - **Where it is most needed, from this session's evidence:** the Salvage Bay overall, its Rules tab specifically (auto-salvage and the confirm interlock are the least self-explanatory things in the release), and the queue panels generally, where a player meets lanes, queue depth and reservations as new vocabulary all at once.
  - ⚠️ **A `?` BUTTON EXPLAINS A MESSY PANEL, IT DOES NOT FIX ONE.** The user's own words on the Rules tab were that it "feels a little messy" and "may need a little work". If that is true, contextual help is a band-aid over a layout problem. Do BOTH, in this order: tidy the panel first, then explain what genuinely remains. The risk otherwise is that the button becomes the reason not to simplify, which is how a screen ends up needing an explanation permanently.
  - Pairs naturally with the 0.13.5 tooltip and accessibility work, and with the icon-pack sweep, since all three are presentation-layer passes over every screen. Doing them in one pass per screen is cheaper than three visits.

- **⭐ AUTOMATION RULES ALSO BELONG UNDER OPTIONS, IN A GAMEPLAY TAB (user, 2026-09-10; for 0.13.5).** The auto-salvage rules currently live only on the Salvage Bay's Rules tab. The user wants them reachable from **Options** as well, under a new **Gameplay** sub-tab. Note the word: **also**, not instead. Two entry points, one setting.
  - **Why it is the right call, from this session's evidence:** the user could not find their own auto-salvage rules, because a tab rail above the scroll position on a phone is effectively invisible. The two entry points serve two different mental models: the **Salvage Bay** copy is CONTEXTUAL, found by a player already thinking about salvage, while the **Options** copy is where someone goes when they think "there must be a setting for this". Neither alone covers both.
  - ⚠️ **BOTH MUST BE VIEWS OF ONE STATE, NEVER TWO COPIES.** The rules live in the save (`state.autoSalvage`), deliberately, because the tick reads them and a localStorage-only rule would behave differently offline. So both surfaces must read and write that same field and nothing else. The failure mode to avoid is a settings screen that snapshots on open and writes back on close, which silently clobbers a change made on the other surface. Render both from the same derivation and the problem cannot arise.
  - **A Gameplay tab is a good home for more than this.** The confirm-by-quality preference is the obvious companion, since it is the interlock that governs what auto-salvage may touch and it is currently only on the Rules tab too. Worth deciding what else is a gameplay SETTING rather than a facility control while the tab is being designed, instead of accreting into it one release at a time.
  - Pairs with the 0.13.5 Options and accessibility work and with the contextual-help button above; all three touch the same screens.

- **⭐⭐ INVISIBLE AFFORDANCES: sweep every tap-the-text control and give it a real button (user, 2026-09-10; for 0.13.5). This is a CLASS, not three tickets.** Three instances surfaced in one conversation, and the user's own words on the ship one are the whole argument: **"no one knew how to rename ships until I told them to tap the ship name"**.
    1. **Ship rename** (`renameShip`): click-to-edit on the ship name in the ShipSystemsPanel header. No affordance.
    2. **Captain rename** (`renameCaptain`): the same pattern, shipped in the same release. Almost certainly equally undiscoverable, and it was never separately reported only because nobody found it either.
    3. **Facility description**: the user proposed tapping the SCREEN NAME to show it, which would have added a fourth instance of the same problem rather than fixing it. See the contextual-help entry: prefer a visible `?` button.
  - **The rule to apply:** if an action is only reachable by tapping something that does not look like a control, it does not exist for most players. A visible affordance (a small pencil or edit button beside the name, a `?` beside a title) costs almost nothing and is the difference between a feature and a secret. Keep the tap-the-text shortcut for people who know it; just never let it be the ONLY route.
  - **Do it as ONE sweep.** This project has twice paid for fixing instances instead of shapes (a trimmed-space bug that hit 9 sites after 2 were fixed, and a zero-manifest guard fixed in one place that recurred in another). Grep for click and keydown handlers on text elements, headings and name spans across every panel, enumerate them, and give each a visible control in one pass. Expect more than the three above.
  - ⚠️ **ACCESSIBILITY OVERLAP, and it is not a coincidence.** A clickable element that is not a button is usually also missing a role, a keyboard path and a focus state; the two pre-existing `RadialWeb.svelte` a11y warnings are exactly this shape (`div` with a pointer handler and no ARIA role). So this sweep and the 0.13.5 accessibility pass are largely the SAME work: converting a tappable text node into a real `<button>` fixes discoverability, keyboard access, focus visibility and the lint warning in one edit. Do them together.
  - Pairs with the contextual-help `?` button, the tooltip rework and the icon sweep, all of which touch the same screens in 0.13.5. One visit per screen, not four.

- **✅ QA DECISIONS RULED, 0.13.3 pre-release (user, 2026-09-10).** Eight decisions from the release's QA sheet, recorded together so the reasoning survives with them.
  - **D1, Shipyard lanes do not add build throughput: CONFIRMED as the existing plan, not a defect to patch.** `BUILD_CONCURRENCY_CAP` is 1, so an extra bay currently adds only repair capacity, and 0.13.3 ships copy that says exactly that. The user's ruling: the **construction-berth / repair-berth split is still the plan** (see the SHIPYARD BERTHS entry). ⚠️ **That work is therefore what raises the build cap**, since construction berths are by definition build capacity. Whoever builds it must change the rung copy back at the same time; the current wording carries an in-code comment saying so, naming the constant.
  - **D2, inconsistent number formatting: FIX, standardise.** "I want everything to be standardized and consistent." Being addressed in 0.13.3 as a display-only sweep across the crafting and salvage consoles. Underlying precision is untouched, since a rounded value must never become the number a gate compares against.
  - **D3, zero-quantity manifest lines: KEEP THEM.** The user's reasoning is worth preserving because it is not obvious: "It shows that you don't have enough of something and doesn't make you think you have to swap over to the warehouse tab to then find you have 0 of something." A zero line is INFORMATION, not noise: it answers a question the player would otherwise leave the screen to answer. ⚠️ This does NOT reinstate the all-zero case: a salvage that recovered nothing still says so plainly rather than printing a row of zeroes under a claim that everything went to the Warehouse, which was a false statement fixed earlier in the release. Partial manifests keep their zeros; all-zero manifests keep their honest sentence.
  - **D4, offline completions show "time not recorded": FIX, must be accurate.** "The jobs completed should reference the amount of time that passed offline." Being addressed in 0.13.3. ⚠️ Constraint recorded for whoever revisits it: do not fabricate a precise timestamp that cannot be supported. Stating elapsed game time and that it happened while away is accurate; an invented exact time is worse than an honest approximation because it looks authoritative.
  - **D5, log rows do not say what was salvaged or gained: FIX, show both.** "What was salvaged. What was gained." Being addressed in 0.13.3, reusing the Salvage Bay panel's existing shaping rather than writing a second formatter, so the two surfaces cannot drift.
  - **D6, combat patrol returns are invisible in the log: DESIGNED, but NOT for 0.13.3.** The user wants a patrol to report **why it came home**, in the shape: *"Completed X combat patrol missions before returning to the homeworld. Reason for returning: X"*, where the reason is something like **ran out of fuel** or **was defeated in combat**. ⚠️ This is a FEATURE, not a copy fix: nothing currently tracks a patrol's termination REASON, so the sim has to record why a run ended and carry it through to the completion record. That is combat and operations work, which makes it 0.13.4 territory alongside the missions patch. Doing it properly also means enumerating every way a patrol can end, exhaustively, so a new ending is a compile error rather than a blank reason.
  - **D7, inventory tiles have no accessible label: FIX in 0.13.5.** "100% should get a proper accessible label for sure. 0.13.5 is the right call." Folds into the accessibility pass and the invisible-affordances sweep, which are largely the same edit.
  - **D8, only Facilities uses SVG icons: FIX in 0.13.5, placeholders acceptable.** "All tabs should use SVG for sure... A quick pass can replace all UI emoji with SVG graphics. Even if it's some sort of placeholder." ⚠️ **The placeholder permission applies to the ART, not to the ARCHITECTURE.** The icon-pack plan makes the registry the load-bearing part: every glyph resolved through a named registry, swappable at runtime, with a defined fallback. Placeholder artwork behind a correct registry is fine and can be replaced later for free. Hundreds of hardcoded call sites behind pretty art is the thing that has to be unpicked when packs arrive. Build the seam, fill it with whatever art is to hand.

- **✅ 0.13.4 AND 0.13.5 STAY SEPARATE RELEASES (user decision, 2026-09-10).** The user asked whether to combine them, since 0.13.4 looked small, and then ruled against it after the evidence: **"0.13.4 should still be its own beast. A smaller patch, but still its own. Infrastructure under the hood and so forth. Then 0.13.5 is its own beast still as well."**
  - **0.13.4 = a SMALLER, INFRASTRUCTURE release.** Under-the-hood systems work. Its current contents: the transit-berth / turnaround docking slots, D6's patrol termination reasons, and the candidates parked at "0.13.4 or later" (the lane-allocation model, per-facility queue-depth talents).
  - **0.13.5 = the presentation and architecture release**, unchanged: Options reorg, the global readability pass, the desktop/mobile presentation split, accessibility, the icon registry sweep, the tooltip rework, the `?` help button and the Options Gameplay tab.
  - **WHY COMBINING WAS REJECTED, so the question does not get re-opened from the same premise.** Three reasons, all evidenced rather than preference:
    1. **0.13.5 is not the small half, and it has GROWN.** Its own entry already warned "the a11y pass makes 0.13.5 meatier than a pure menu reorg, split if it balloons", and that was written BEFORE the icon registry, tooltip rework, `?` button, Gameplay tab, invisible-affordances sweep and accessible labels were added to it. It also contains the mobile/desktop presentation-layer split, which is architecture.
    2. **0.13.4 is not as small as it looks.** The Ops tab was scoped as a light readability tidy, but 0.13.4 also holds the docking system (which carries a real softlock risk, see the TWO KINDS OF DOCKING SLOT entry) and patrol termination reasons, which need the sim to record WHY a run ended. That is systems work, not a tidy.
...
- **⭐ STANDARDIZE THE PATCH-NOTE FORMAT (user, 2026-09-10, "a thing for future me").** Every release so far has had its notes written fresh, and they have drifted: 0.13.3's first draft was long enough that it did not fit a Discord post and had to be rewritten by hand after the fact. There are effectively THREE audiences and they want different lengths, which is the actual problem: the **in-game Patch Notes tab** (long, complete, one paragraph per release, the current `PATCH_NOTES` shape), the **landing page news strip** (short), and a **Discord post** (hard 2,000 character ceiling, skimmable, bold headers).
  ...
  - Cheap to do at the start of any release rather than at the end, since the format is what the notes get written INTO.

- **✅ 0.13.4 SCOPE LOCKED (user, 2026-09-11): all four infrastructure items, no UI work.** The user confirmed **all four** parked items ship together: the **transit-berth / turnaround docking slots**, **patrol termination reasons** (QA item D6), the **lane-allocation model** (free lanes join a running order once nothing new is waiting), and **per-facility queue-depth talents** (shared trunk, then branches).
  - ⚠️ **THE LANE-ALLOCATION MODEL IS THE HEAVY ONE, and it is parity-critical.** It separates the WORK from the LANE: today a craft line IS a lane, with `remaining` living on the lane, which is exactly why a batch cannot spread. It changes WHAT RUNS ON WHICH TICK, so offline-equals-live must be re-proven from scratch rather than assumed to carry. Build it as its own gated phase so it can peel if it grows, the way 0.13.3's phases were structured.
  - ⚠️ **THE DOCKING SYSTEM HAS THE SOFTLOCK SHAPE.** "You cannot unload without a free turnaround slot" is structurally the 0.11.1 docks bug that bricked saves. Design the escape valve FIRST, not in QA. `shipBuildSlotCount`'s `bays - 1` repair reservation is the in-repo precedent for exactly this class; copy it rather than inventing.
  - **THE OPS / LOGISTICS PRESENTATION TIDY GOES TO 0.13.5, not here.** The user: "I haven't been emphasizing a ton of UI changes (only the glaringly obvious things like spacing and such), and left anything else to this patch", where "this patch" is the UI release. So 0.13.4 is genuinely infrastructure-only, and 0.13.5 homes in on look and feel across everything at once. This closes the question the previous entry left open.

- **⚠️ THE TOOLTIP EPIC IS UNSLOTTED, AND I CAUSED CONFUSION ABOUT IT (correction, 2026-09-11).** The user asked whether the standardized tooltip system was slated for 0.13.x or 0.14.x. The honest answer, from the record:
  - The tab-by-tab roadmap says it is **STILL UNSLOTTED** and "needs its own point release(s) or to be folded where they fit".
  - An earlier entry (2026-08-27) DID slot it: "0.13.1 = QoL bundle + tooltip epic". **That slot no longer exists**: 0.13.1 became Command Home when the series was re-split into the tab-by-tab cadence, and the epic fell through the gap. So the user's memory that it was "during the 0.13.x series" is correct in spirit; the release it was assigned to turned into something else.
  - ⚠️ **I repeatedly wrote "the 0.13.5 tooltip rework" in entries logged during 2026-09-10 as though it were decided. It was not.** That is an assumption propagated into the record, the same failure mode that caused the 0.13.3 queue-coverage round trip. Treat those mentions as UNDECIDED until this entry is resolved.
  - **RECOMMENDATION: fold it into 0.13.5 and accept that this makes 0.13.5 the big one**, explicitly allowed to peel the way 0.13.3 was. The epic is a standardized template set plus a shared opaque-surface token, which is structurally the SAME KIND of work as the icon registry already slotted there. Both are "build the shared presentation primitive", and doing them apart means two visits to every screen. That reframes 0.13.5 as the PRESENTATION-PRIMITIVES release: icons, tooltips, type scale, theming, accessibility.
  - ✅ **RESOLVED (user, 2026-09-11): the tooltip epic goes into 0.13.5.** Their reasoning, which is the deciding argument: **"Most of the remaining tab changes are going to be cosmetic in nature after we finish with 0.13.4, so 0.13.5 is the right place to put it."** With 0.13.4 taking all the systems work, what remains across the tabs is presentation, so 0.13.5 stops being an Options release and becomes the PRESENTATION-PRIMITIVES release: icons, tooltips, type scale, theming, accessibility, plus the deferred Ops/Logistics tidy. The user acknowledged it is a big one. ⚠️ Therefore build it in GATED PHASES that can peel, exactly as 0.13.3 was structured, and treat the earlier ballooning warning on this entry as active rather than historical.

- **⭐⭐⭐ 0.13.6: THE ITEM LIFECYCLE REWORK (Blanks, the rolled pool, the Armory, and the Museum). Consolidated 2026-09-11 from one design conversation; supersedes the earlier scattered Armory notes.** This is one system with four parts, not four features. It is the largest design in the 0.13.x line and it earns its own release.

  ### The core idea: crafting stops producing instances

  **Today:** you craft a ship system and it rolls everything immediately, producing a unique instance that consumes a warehouse slot. Every instance is unique (iLevel, quality, rarity, durability), so nothing stacks, so storage grows without bound and auto-salvage exists mainly as a pressure valve on it.

  **Proposed:** crafting produces a **BLANK**. A blank is not instantiated, so every blank of a given type is identical, so **blanks STACK exactly like raw ore**: one icon, one count, one slot for a thousand of them. The user's own framing for why ship systems differ from ore today: "each ore isn't unique". Blanks make them not-unique until you choose otherwise.

  You then **inspect** a blank ("right click to inspect your team's work"), which **instantiates** it and performs the rolls from your crafting capabilities, with the same odds as today. The rolled item moves to the pool of instantiated items.

  ⭐ **WHY THIS IS MORE THAN A STORAGE FIX, and the reason the whole release hangs together:** it removes the ceiling on how many attempts you can bank. Today you are limited to crafting a batch at a time. With blanks you can craft millions, hold them, and roll when you choose. The user: "eventually, completion will require millions of all of the things". The Museum wants max-score items, max scores need perfect rolls, perfect rolls need many attempts, and blanks are what make banking attempts possible. **The storage saving is a side effect; the completionist loop is the point.**

  ### It is a LIFECYCLE, not three inventories

  Describe it as three STAGES of one life, never as three parallel containers. "Where is my thing" then has a real answer (wherever it is in its life) instead of being a scavenger hunt:
  1. **BLANKS** (unrolled). Stackable, fungible, warehouse-side. Crafting deposits here.
  2. **ROLLED POOL** (instantiated, uncommitted). One slot each, because no two rolled items are alike. **This is the existing Ship Systems warehouse tab, repurposed.** From here an item can be auto-salvaged (after the grace period), donated (a future reputation system), or committed to the Armory.
  3. **ARMORY** (committed). An item equipped into a layout slot lives here and is **removed from the warehouse**.
  - ✅ **AUTO-SALVAGE'S JOB SHRINKS TO STAGE 2 ONLY**, which is a real simplification: blanks are not rolled so they are not salvage targets, and Armory items are protected by being in a layout. The automation stops being a pressure valve on everything and becomes a curator of exactly one stage.
  - ⚠️ **THE GRACE PERIOD MOVES.** 0.13.3.1 stamps the grace at CRAFT. Under this system crafting produces a blank and instantiation happens later, so the safe window must start at **INSTANTIATION**, not at craft. Whoever builds this must move the stamp; 0.13.3.1 is correct for today's model.

  ### The Armory

  - **Slot count is DERIVED, not flat:** **layout slots x the total item slots a ship has** (maximum hardpoints and so on, even where a given hull cannot use them all). The user's "10 tactical, 10 prospector, 10 explorer" plus 5 custom describes **LAYOUTS**; each layout then holds a full ship's worth of items. So total item capacity is layouts times per-ship slots, which is much larger than the layout count and should be computed, never hardcoded.
  - ⚠️ **FOUR SPECS EXIST, NOT THREE.** `ShipSpec` is `"general" | "prospector" | "tactician" | "explorer"`. The user enumerated tactical, prospector and explorer, omitting **general** (the General Freighter's spec). **OPEN: does general get its 10 layouts (40) or not (30)?** Also the code spells it **`tactician`**, not "tactical"; match the data.
  - **Slots are CHECKED OUT** by the ship or crew member using them, so a slot is a binding rather than a storage cell. More slots come from talents.
  - **Custom layouts are spec-agnostic and exist for CHALLENGE content**, which is why they are worth building: the user's example is clearing a tier X mission with a prospector ship not built for combat.
  - ⚠️ **DECIDE BEFORE BUILDING: what happens to a checked-out slot when its ship is SALVAGED?** A binding whose owner is gone is the classic way gear gets stranded or silently dropped, which the never-silently-delete rule forbids. Options: the slot releases and its gear returns to the pool (safest, recommended); the slot persists as an orphan the player must clear (visible but annoying); or salvage is refused while a slot is checked out (safe, but a softlock shape). Same question for a dismissed crew member once crew exists (0.15.0).
  - Because the Armory absorbs committed gear, the user intends to **greatly increase warehouse space for ship systems**. Note the interaction with the equipment cap and the 0.11.1 softlock escape valve: nothing here may become a way to exceed a cap invisibly, and a full Armory must never strand an item.

  ### The Museum (working name; a lore-accurate name is coming)

  - **One slot per craftable item**, graded on what you put in it. A completionist display, not storage.
  - **Scoring: rarity x quality x iLevel x 10.** Every item has a knowable MAXIMUM score, which is what makes a completion percentage possible. **Rolls and durability deliberately do not count.**
  - ⚠️ **THE FORMULA HAS A ZERO PROBLEM, and the user's two examples disagree.** Quality starts at Q0 and `rarityIndex` starts derelict = 0, so a raw multiply scores **ZERO** for any Q0 or derelict item. Their "10 points for a q0 item at ilevel 1" only works **1-INDEXED**: `(0+1) x (0+1) x 1 x 10 = 10`. Their "Q2 ilevel 100 augmented = 2 x 100 x 2 x 10 = 4000" uses **raw** indices; 1-indexed it would be 9000. **RECOMMEND the +1 form**, because it makes the Q0 example work and because a slot that scores literally nothing reads as broken rather than modest. OPEN: confirm.
  - ⚠️ **Luminous and constellar share `rarityIndex` 5** (deliberately parallel flavors at one power tier), so they would score identically. Fine for a power metric, possibly odd for a COLLECTION metric where a completionist expects distinct entries. OPEN.
  - ⚠️ **The denominator must be the ABSOLUTE tier ceiling, not the player's current cap.** Max iLevel depends on crafting level, so computing "maximum score" from what a player can currently make would move their completion percentage as they LEVEL rather than as they COLLECT. Fix it to the hard cap.
  - OPEN: does slotting an item into the Museum **consume** it or merely **record** it? Consuming makes the axis expensive and permanent; recording makes it cheap and re-checkable. Those play very differently.

  ### ⚠️ SAVE-SCUMMING IS THE STRUCTURAL RISK, and it is new

  Today rolls happen inside the tick with a seeded rng and are baked into the save before the player sees anything. Moving the roll to a **player action** means a player can inspect, dislike the result, reload, and roll again. The save is in localStorage, so that is a two-second exploit and it would hollow out the entire rarity ladder.
  - ⚠️ **The obvious fix conflicts with stacking.** Giving each blank its own seed kills the exploit but makes every blank unique, which destroys the fungibility the whole design rests on.
  - **RECOMMENDED: a rolling seed on `GameState` that advances per inspect.** Reload and the next roll is identical, because the seed has not advanced. Stacks stay intact. Residual weakness: a player could reload and inspect a DIFFERENT blank to redirect the roll; close it by deriving the roll from the seed AND the item type, so redirecting gains nothing.
  - Whatever is chosen must stay a pure function of the save, because that is the same property the offline-equals-live economy depends on.

  ### Other open questions

  - ⚠️ **Do blanks capture crafting level at CRAFT time, or read it at INSPECT time?** The user's wording ("rolls based off of your crafting capabilities") reads as inspect time, which is what keeps blanks fungible and stackable. Consequence to accept deliberately: **banking blanks early and opening them at max level becomes strictly optimal**, so early crafting produces nothing usable until the player chooses to open. That is a fine idle-game shape (bank now, open stronger later) but it should be a choice, not a discovery. The alternative, capturing level at craft, fragments the stack by level and largely defeats the design.
  - **Blanks need Decimal counts**, like materials, since the user expects millions. Confirm they ride the existing inventory machinery rather than a parallel structure.
  - **Donation** is named as a future reputation-system hook. Log only; not this release.
  - Naming: "Museum" is a working name pending a lore-accurate one.
  - ✅ **USER RESPONSE ON SAVE-SCUMMING (2026-09-11): it goes away at 0.14.0, when saves load from the SERVER, and 0.16.0 carries a save wipe.** Recorded, with two caveats that do not change the plan but should not be lost:
    1. ⚠️ **0.13.6 ships BEFORE 0.14.0**, so there is at least one release where the exploit is live. Single-player with no leaderboards makes that mostly self-harm, BUT 0.14.0's own plan includes a ONE-TIME local-save import into an account, so a scummed save would carry its inventory into the online era and sit there until the 0.16.0 wipe.
    2. ⚠️ **SERVER-LOADED IS NOT SERVER-AUTHORITATIVE.** Loading a save from the server stops file editing, but if the roll is still COMPUTED CLIENT-SIDE and reported upward, a client can still influence it. Real anti-cheat needs the roll itself server-side, which this project's own notes already flag as the one hard retrofit because it needs an authoritative economy.
  - ⭐ **THEREFORE THE ROLLING SEED IS NOT A STOPGAP, IT IS THE FOUNDATION FOR THE ONLINE CASE.** A roll derived deterministically from saved state is one a server can RECOMPUTE AND VERIFY rather than trust. So the work is not discarded at 0.14.0; it is precisely what makes verification cheap later. It is also small (a seed field plus an advance per inspect), so deferring it buys little while making the feature's integrity depend on a much larger release landing on schedule. **Recommend building it with 0.13.6 regardless of the online timeline.**

- **📐 STANDING PRINCIPLE: CONTENT-DRIVEN UI. New content appears by being added to DATA, never by editing the UI (user, 2026-09-11: "Modularity and abstract design are key. Once items are available for use, the system should just allow for the ability to show those items and so forth").** Not an idea to schedule; a rule to apply to everything from here.
  - **The test:** if adding a rarity, a facility, a ship hull, an item, a talent or a process kind requires someone to remember to also edit a list in the UI, the design is wrong. It should either render from the data, or fail to compile.
  - **The enforcement that makes it real is exhaustive `Record`s over unions.** A total record forces every member to be answered, so a new member is a BUILD FAILURE rather than a silently missing option discovered months later by a player. This project already relies on it in several places: `QUEUE_ADAPTERS` and `ENQUEUE_PROBE` (a new queueing facility cannot be half-added), `PROCESS_XP_AWARDS` / `PROCESS_COMPLETION_LOG` / `COMPLETION_KIND_VIEW` (a new `TimedProcessKind` must be answered everywhere), `AUTO_SALVAGE_RARITIES_NONE` and the protection-reason record (0.13.3.1), the salvage duration multiplier tables, and `IconName` derived from data via `as const satisfies`.
  - **Corollary, learned the hard way in 0.13.3.1:** when a display must show LESS than the model (for example hiding rarities the generator cannot yet produce), narrow the DISPLAY and keep the MODEL total. Narrowing the model is the tempting shortcut and it silently undoes the modularity, because the day the content becomes available someone has to remember to re-widen it, which is exactly the remembering this rule exists to eliminate.
  - **Applies directly to the queued work:** the 0.13.5 icon registry (every glyph through a named registry, swappable, with a fallback, so an icon pack is data), the 0.13.6 Armory slot counts (derived from `SHIP_TYPES` and spec counts, never a literal), the Museum's per-item slots (one per craftable item, enumerated from the item table), and per-facility queue-depth talents.

- **⭐ A DEDICATED SALVAGE LANE FOR EQUIPPABLE GEAR (user, 2026-09-11; for the exploration patch or a release between 0.14.0 and 0.15.0).** A Salvage Bay lane reserved for ship systems and other equippable gear, distinct from the general lanes that also chew salvaged materials, with auto-salvage queueing into it.
  - ⚠️ **FIRST, A CORRECTION TO THE PREMISE, so nobody rebuilds what exists.** The user said "salvaging items currently is instant, but salvaging items should be identical to the way that salvaging queued items works". **Equipment salvage is ALREADY a timed, queued job** as of 0.13.3: 60 ticks base, scaling with iLevel and quality, promoted through the bay like any other order, and auto-salvage already feeds that queue rather than destroying anything directly. What IS still instant is **HULL TEARDOWN**, deliberately, and the code says so: "HULL teardown is deliberately still instant and still a Ships-tab action." So the only genuinely new part of the request is the DEDICATED LANE.
  - **WHY A DEDICATED LANE EARNS ITS KEEP LATER even though it is unnecessary today:** 0.15.0 brings ship crew and away teams, and therefore **crew equipment**, which is a second class of equippable gear. At that point two equipment classes plus salvaged materials all compete for one undifferentiated bay, and a player curating gear can be starved by a large material batch (or the reverse). A reserved lane makes the guarantee explicit instead of leaving it to queue order.
  - **This is the same shape as two patterns already in the repo**, so copy rather than invent: `shipBuildSlotCount`'s `bays - 1` reservation (a class of work that can never be crowded out) and auto-salvage's own manual-depth headroom (the automation never takes the last slot). Both exist precisely to stop one consumer starving another.
  - **Auto-salvage will need to change with it**, since it currently reasons about one undifferentiated bay. Note that the 0.13.3.1 protection-reason seam already generalises the "why can this not be taken" half; the lane question is the "where does it go" half.
  - ⚠️ **Interacts with the 0.13.6 item lifecycle rework.** Under that design auto-salvage only ever curates the ROLLED pool (blanks are unrolled and Armory items are committed), so the volume reaching the bay changes substantially. **Sequence this AFTER 0.13.6**, or the lane is sized against a flow that is about to be replaced.
  - Owner: the exploration patch, or a release between 0.14.0 and 0.15.0, per the user. Not now.

- **✅ BUILT, 2026-09-11: THE AUTO-SALVAGE TERMINAL (this supersedes the entry directly above).** The entry above asked for a lane reserved by TARGET TYPE (equippable gear). The user redirected mid-build to a split by ORIGIN instead, which solves the same problem and generalises better: whatever the automation is ever pointed at, including 0.15.0's crew equipment, it runs on its own pipeline. What shipped:
  - **`state.autoSalvageQueue`**, a SEPARATE unbounded array the rules write to. The automation cannot consume, reorder or appear in the player's queue depth, because it is not in the container any of those are computed from. SAVE_VERSION 43 -> 44 with `MIGRATIONS[43]` for the new field.
  - **One Terminal lane** (`AUTO_SALVAGE_TERMINAL_LANES = 1`), auto-only, plus the borrow rule: auto may also take a GENERAL lane that has been idle for `AUTO_SALVAGE_BORROW_IDLE_SECONDS` (10s of game time). Manual work is promoted first within every tick, and nothing is ever preempted.
  - **`AUTO_SALVAGE_MANUAL_HEADROOM` was REMOVED**, along with its depth-1 special case, because both existed only to referee a competition that no longer happens.
- **A TERMINAL LANE UPGRADE TRACK, AND SPEED TALENTS FOR IT (deliberately NOT built, 2026-09-11).** The user wants both eventually. Neither is built and the recommendation is below, so the decision is made with findings rather than in the abstract.
  - **RECOMMENDATION ON THE LANE TRACK: probably do not build it, or build it only after the borrow rule has been played with.** The borrow rule already makes auto-salvage scale with the general lane track the player is buying anyway, so a second concurrency track would sell the same benefit twice and split one clear decision ("buy salvage bays") into two fuzzy ones. It would also make the Salvage Bay the only facility in the game with two separate lane tracks, which is a fair amount of console surface for a number that is already moving. The honest test to run first: does auto-salvage actually fall behind with 2 to 3 general lanes and the borrow rule on? If it does not, the track is a solution to nothing.
  - **RECOMMENDATION ON SPEED TALENTS: yes, and there is already a home for them.** Speed is the axis the borrow rule does NOT cover, so it is the one that would actually move. It belongs in `salvageJobDurationTicks`, where every other duration modifier already lives, so a talent is a multiplier in one function rather than a second code path. ⚠️ It must be a modifier the offline resolver can read from the save (a Homeworld talent is, like every existing one), and the duration is fixed at job start, so a talent learned mid-teardown correctly does not retune work already running.
  - Raising the Terminal lane count in the meantime is a ONE-LINE data change on the named constant; nothing else reads a literal.
- **A MANUAL "CLEAR" OR PER-ORDER REMOVE FOR THE TERMINAL QUEUE (2026-09-11, small).** Today the only way to clear pending auto orders is to switch the rules off, which clears the whole Terminal queue (that clear is new, and it is a protection: against an unbounded queue, hand-removing a hundred pending destructions is not a reasonable ask). A per-order Remove, or a Clear button that does not require toggling the rules, would be a small, purely additive console affordance. Not urgent: nothing is destroyed by a pending order, and the rules-off clear covers the case that matters.

- **⭐ THE QUARTERMASTER: a free source of Standard-Issue equipment, as structural anti-softlock (user, 2026-09-11).** A merchant or quartermaster who provides Standard-Issue gear for **0 credits**. The user's framing: "a way around this... that's something else that is needed, and could be a great option for anti-softlock."
  - ⭐ **WHY THIS BEATS THE WARNING DIALOG IT WOULD REPLACE.** 0.13.3.1 makes Standard-Issue baselines auto-salvageable behind a confirmation warning. A warning says "do not do this"; a free refill makes DOING it recoverable. That removes the failure mode instead of labelling it, which is the project's stated value (peace is the product, never build in punishment) applied structurally rather than as a caution sign. With a quartermaster, stripping every baseline stops being a soft-block and becomes an errand.
  - **THE CHEAP VERSION NEEDS NO MERCHANT AT ALL:** an empty slot on the Ships loadout offers **"Install Standard-Issue"**, minting one free. It sits exactly where the player needs it at the moment they need it, and is roughly the same work as the dialog. A full quartermaster FACILITY can follow later if the fiction wants a place rather than a button. Recommend the button first, the place later.
  - ⚠️ **CHECK THE COMBAT INTERACTION BEFORE BUILDING.** Economy baselines are stat-neutral (+0), so giving them away costs nothing. **Combat baselines are NOT**: Standard-Issue plating and shields carry real values, and `canDispatchPatrol` deliberately blocks a patrol when a required combat slot is empty. Free combat refills would make that blocker trivially bypassable. That may well be fine, since the blocker's purpose is "do not fly naked" rather than "grind for a floor", but it must be a DELIBERATE call. Options: free for economy slots only; free for all, accepting the blocker becomes advisory; or free with a cooldown.
  - **Interacts with 0.13.6's item lifecycle rework:** if crafting produces stackable BLANKS, a free baseline is a different kind of object from a blank (it is already instantiated and stat-fixed). Decide whether the quartermaster hands out instantiated baselines or blanks; instantiated is almost certainly right, since the whole point is an immediate floor rather than a roll.
  - Also worth considering as the reputation-system's first hook, since the user has already floated donating salvage to a future reputation system and a quartermaster is a natural counterpart.

- **⭐ CONFIRMATION PRESETS: one control that sets every dialog toggle (user, 2026-09-11; for 0.13.5, alongside the Options Gameplay tab).** The game is accumulating confirmation dialogs (salvage confirm-by-quality, the batch confirm, the no-grace warning, the baseline warning) and each currently carries its own "do not show again". The user wants a preset selector that checks and unchecks them as a set.
  - **The presets, in the user's order:** **All Enabled**, **Tutorial**, **Beginner**, **Intermediate**, **Advanced**, **All Off**. Selecting one sets the individual toggles.
  - ⭐ **PRESETS MUST SET THE CHECKBOXES, NOT REPLACE THEM.** After picking Beginner a player can still flip one dialog individually, and the label must then say so ("Beginner, modified", or flip to "Custom"). A preset control that keeps claiming a preset the player has since edited is the classic way these systems start lying, and it is exactly the class of stale-label bug this project has already fixed several times.
  - **A HELP BOX SITS NEXT TO THE SELECTOR** detailing what each preset includes and excludes. The user was specific that it belongs right next to the option rather than buried in the manual, which is the same instinct behind the contextual `?` button logged separately; build them together.
  - **TUTORIAL HANDOFF:** at the end of the tutorial, tell the player that if the confirmations feel restrictive, the Options menu can change them. That is the moment a player has just met the most dialogs and is most likely to want fewer. ⚠️ **DEPENDENCY, verify before planning: does a tutorial actually exist yet?** The "Tutorial" preset and the end-of-tutorial handoff both assume one. If not, the preset still makes sense as a named safety level, but the handoff has nowhere to live.
  - **Interaction with the baseline warning (0.13.3.1):** the user suggested that warning might become tutorial-mode-specific once the Quartermaster ships and makes the situation recoverable. Recorded as an option rather than a decision: the warning is informational rather than load-bearing once a free replacement exists, so demoting it is defensible, and it is a one-condition change either way.
  - ⚠️ **"All Off" disables destructive confirmations too**, which is a deliberate player choice and consistent with the project's stance of informing rather than restricting. But it should be the explicit, clearly-labelled end of the scale rather than something a player lands on by dragging a slider, and the help box should say plainly what it turns off.
  - Storage: these are per-device VIEW preferences that do not change what the tick does, so `localStorage` via `safeStorage` is correct, matching the existing confirm-preference modules. The preset itself is one more stored value, not a second source of truth.
  - ✅ **INTERACTION MODEL, refined by the user (2026-09-11):** individual checkboxes AND a preset dropdown, side by side. Editing any checkbox flips the dropdown to **Custom**. Changing the dropdown raises a confirmation asking whether to overwrite the current settings with that preset; **Apply** overwrites, **Cancel** reverts the dropdown to what it was.
  - ⭐ **ONLY CONFIRM WHEN SOMETHING WOULD ACTUALLY BE LOST.** If the selector currently reads an UNMODIFIED preset, switching to another preset destroys nothing, so the dialog is pure friction and is exactly the nagging this whole feature exists to reduce. Confirm only when the current state is **Custom**, where hand-tuned settings really would be overwritten. That also keeps the dialog meaningful on the occasions it does appear, instead of training players to click through it.
  - ⚠️ **THE PRESET CONFIRM IS NOT ITSELF A MANAGED DIALOG.** It is a control-level guard on the settings screen and must not appear in the list the presets govern. Otherwise "All Off" switches off the protection on the very control that sets "All Off", which is both a recursion and a way to lose custom settings in one click.
