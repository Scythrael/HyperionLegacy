...
**Updated:** 2026-07-18, FEATURE-COMPLETE 0.11.0 PROMOTED TO PRODUCTION (`origin/main` = `origin/staging` = `75d9d86`). Very long session. Read this FIRST, then the memory files and the docs it points to.

---

## 0. TL;DR (read this, then the sections you need)

- **Production (crystalisoft.com) is at 0.11.0** (`origin/main` = `75d9d86`, promoted 2026-07-18, prod == staging). Public. Do NOT touch main without a fresh explicit user go-ahead.
- **NEXT (user-directed 2026-07-18, do IN THIS ORDER):** FIRST the recommended CLEANUP as careful, separately-tested refactors on now-LIVE code: (1) `foldXpLevelUps` DRY refactor (consolidate the three subtract-and-carry XP loops: captain `tick.ts:~1075`, FA `~1184`, crafting `~1236`, into one shared helper, prove no behavior change), (2) converge the FA-XP BLACKLIST (`tick.ts:~4701`) and crafting-XP WHITELIST (`~4724`) so a future `TimedProcessKind` can't silently inherit one but not the other. THEN **0.11.1** (below), which ALSO includes a **fitment -> "install ship systems" terminology sweep of ALL user-facing text** (code vocab stays internal; see the 0.11.1 notes doc).
- **NEXT: nothing is in flight.** The next feature is the user's call: **0.11.1 = UI restructure** (Quartermaster facility split, material recategorization, salvage-as-own-facility, salvage RESULT display + tiered/configurable salvage confirmation, Help tab, desk-OS lens; see `docs/plans/2026-07-18-0.11.1-ui-restructure-notes.md`, the user's CONDITION for shipping 0.11.0's tabs in the Warehouse). Also open: the ship-class NAMING threads (see `2026-07-18-ship-class-naming-notes.md`, Frigate/small-ship names + Dreadnought theme still unpicked), Material Lines (`feat/material-lines-0.11.1` branch, RENUMBER to 0.11.2), and 0.12.0 combat (HARD PREREQ: captain ids to a monotonic counter first).
