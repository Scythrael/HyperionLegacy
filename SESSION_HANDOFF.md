...
## TL;DR
- **Branch:** `feat/combat-0.13.0`, tip **`a3835fa`** (+ this handoff commit). Tree clean, all committed.
- **Gate now:** `npm run check` = 0 errors (2 pre-existing RadialWeb a11y warnings, ignore); `npm test` = **1579 passing**; `npm audit` = 0 vulnerabilities; `npm run build` OK.
- **Deploy state:** devpreview (`origin/staging`) = latest tip (the full combat build + 2026-07-29 bug-hunt + online/cloud-save security hardening, live for the user to test). **PROD (`origin/main`) = `e282614` = 0.12.1 + hotfixes, UNTOUCHED.** Combat ships as 0.13.0 only after P14 fold-in + user device-test + explicit go.
