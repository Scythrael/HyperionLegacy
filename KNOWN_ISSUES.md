...
## Online / cloud-save security pass, 2026-07-29 (Opus, while user away)

Forward-looking security review ahead of the post-combat online work (Clerk accounts + cloud save + real-time chat). Full threat memo: **`docs/plans/2026-07-29-online-cloud-save-security-design.md`** (read that first for the design-time requirements). No secrets found in the repo (all matches were future-backend docs). `npm audit` clean after the postcss fix (`c6e27f5`).

### FIXED this pass (client-side hardening; all no-ops on a valid save)
- **Save-load hostile-input hardening** (`e97d0b0`): try/catch backstop in `loadFromLocalStorage` (type-confusion brick -> recoverable corrupt-save), `tickDurationSeconds` reset-to-1 (crafted 0 caused an offline-catch-up infinite-loop HANG), `__proto__`/`constructor`/`prototype` key filter + non-object fail-open in the hydration map loops (prototype-pollution defense-in-depth), and `MIGRATIONS[30]` reduce instead of arg-spread (huge-array RangeError / decompression-bomb). Builds on the earlier `deserialize` shape guard (`1f793e1`). Regression tests added.
- **npm audit** (`c6e27f5`): postcss high-severity advisory resolved (build-time dep, low real-world impact here). 0 vulnerabilities.

### DEFERRED, design-time (server must own; see the memo for detail)
- **Client-authority / anti-cheat:** the client computes the whole economy + owns the save. Fine for single-player + honor-system cloud save (PvE, only-hurts-yourself, an ACCEPTED documented risk). REQUIRED before any competitive feature (PvP/leaderboard/shared drops): server-side validation or re-simulation. The deterministic seeded sim + closed-form offline economy make this feasible, do NOT break determinism / offline==live.
- **Clerk auth:** secret key server-only; verify session server-side on every save endpoint; key saves by the verified user id (anti-IDOR); never trust client-sent identity/role/entitlement; HTTPS + locked CORS + rate/size limits.
- **Save as hostile input (server side):** a full field-level schema validator + Decimal-magnitude bounds + decompression-bomb size caps BEFORE migrate/hydrate on any non-local (cloud/imported) save; validate attacker-controlled `created_at`/`last_saved_at`.
- **Names as UGC:** XSS surface is clean today (all escaped) + captain-name validation is strong, BUT save-import carries `label` verbatim (bypasses validation) -> the server must re-validate every name on store AND load, not just on rename; decide unique-handle vs display-name (impersonation); server-side moderation.
- **Chat (the patch after):** the largest new surface. Escaped-text-only rendering (never `{@html}`); server-validated color palette (no raw CSS); server-issued badges; SERVER-MINTED rare-drop/system broadcasts (a client cannot announce its own loot); server-routed whispers; rate limits + moderation. Ships on the same verified-session + server-authority layer as cloud save.
- **sec. 9.7 retrofit:** the tech spec's cheap-in-v1 prep (nullable save `user_id`, auth-behind-interface, feature flags, entitlement hook) was NOT built (only the theme/CSS-var abstraction exists). Add at the next SAVE_VERSION bump so anonymous->cloud migration is a field populate, not a rewrite.

## Lifecycle-leak + async/race hunt, 2026-07-29 (Opus, while user away)

Two more hunters ahead of the long-lived-connection (cloud save + chat) work. BOTH CAME BACK CLEAN, no bugs found; recorded here so the negative result is durable.
- **Resource/lifecycle:** every setInterval / setTimeout / rAF / addEventListener / ResizeObserver / createObjectURL has a matching teardown; the main game loop is single-instance (no double-speed/double-save); no store-subscription leaks. Two non-bug tidiness notes for the incoming socket lifecycle: `updateDetector.ts` visibilitychange/focus listeners use anonymous handlers with no production teardown API (harmless singleton today), and a `ConsoleTabs.svelte` reactive rAF is uncancelled (self-limiting one-shot). Neither is a leak; left untouched (Rule 15).
- **Async/race/state-integrity:** the tick + autosave + load/import machinery is race-free, built on the invariant that every `state` read-modify-write is fully synchronous (no await between read and write). The old Import-Save autosave-clobber fix (`suppressSave`) holds. Documented the invariant IN CODE at the tick loop + the corrupt-modal/suppressSave coupling (App.svelte) so the upcoming async online work cannot silently reopen re-entrancy / lost-update / double-apply. Two latent-fragility notes captured (N1: adding a dismiss path to the corrupt-recovery modal would strand suppressSave=true; N2: the tick loop + autosave start even on the corrupt-load branch, harmless wasted work behind the modal).
