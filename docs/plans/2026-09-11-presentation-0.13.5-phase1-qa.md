...
> ✅ **28 ITEMS ALREADY CLEARED BY A MACHINE RUN (2026-09-12).** Anything marked ✅ below was
> verified against the real code and does not need your time. ⚠️ **The proportion is LOWER than the
> 0.13.4 sheet**, and that is honest rather than lazy: 0.13.4 was engine work a harness can drive,
> while this release is PRESENTATION. I can check that a token is defined, that a contrast ratio
> clears AA and that a preset writes the right values. I cannot check whether a screen LOOKS right,
> and most of what is left is exactly that.
