...
- **RESPONSIVE, full-width on desktop (hard requirement, user 2026-07-21).** The console must fill the desktop width exactly as the game does today, NOT render as a narrow mobile column centered on a wide screen. The mobile mockups are single-column; desktop uses the extra width. Concretely: overviews lay out multi-column on desktop (e.g. a captain/ship overview can put identity + summary + button set across the width, and grids show more per row), and a summoned panel MAY sit beside the overview on desktop rather than fully replacing it, since there is room, while on mobile the panel replaces the overview as described. Every screen is built mobile-first AND desktop-full-width; neither is an afterthought. **Desktop uses as much of the available width as it has to work with; mobile aims to minimize scrolling wherever feasible** (size overviews to fit, avoid making the player scroll for core actions). (Open item for planning: confirm per screen whether desktop keeps the full-replace swap at a wider size, or shows overview + panel side by side.)
...
- The **ship equipment layout is PRESERVED and REUSED** (user 2026-07-21): the current ShipSystemsPanel paper-doll, the ship image with the equipment slots arranged around it, is the ship console's outfit view. It MAY be refined, but keep the ship-image-and-slots layout; do NOT replace it with a flat slot list (the flat list in the Logistics/Ship mockup frame 2 is superseded by this, the installed-systems section IS the paper-doll).
- The **EquipmentTooltip is PRESERVED unchanged** and reused everywhere an item's stats are shown (hard constraint since 0.11.0, reaffirmed 2026-07-21). Do not restructure it.
