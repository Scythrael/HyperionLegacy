...
export const HELP_TOPICS: HelpTopic[] = [
  ...
  },
  {
    id: "combatView",
    title: "Combat View",
    body: "The Combat View is the watch-the-fight screen, opened with the View Combat Log button on an in-progress patrol in Operations. It replays the patrol's current wave round by round, revealing about one round each second so you can follow the fight as it unfolds (a Skip to End control jumps straight to the result). It is a display only, watching or closing it changes nothing about the run. The Arena shows both ships side by side: hull and shield bars with live values, small square status pips (red for damage-over-time such as Plasma Fire, amber for disruptions such as Targeting Drift, green for buffs, hover for the name and rank), and ship-system condition pips per weapon, reactor, and drive reading Nominal, Degraded, Disrupted, or Offline as durability wears (hover for the label). A carrier also shows one pip per drone (Online, Disrupted, Refabricating, or Destroyed). Between the ships, a range track marks the current distance across the Short, Medium, and Long bands with the current band and engagement phase (detection, intercept, weapons ready, firing) called out, since your chosen stance decides the range you try to hold. Below is the Log-Guided combat log: round dividers and one narrated line per event, colored for crits, damage over time, evasions, shield bleed-through, and kills. A Visual mode toggle is present for a later update; for now use Log-Guided.",
  },
  {
    id: "refining",
    title: "Refining",
