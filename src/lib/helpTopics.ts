...
export const HELP_TOPICS: HelpTopic[] = [
  ...
  },
  {
    id: "salvage",
    title: "Salvage",
    body: "Salvage lets you break down what you no longer need to recover part of what it cost. You can salvage a whole ship at the Docks (in the Drydock program): its installed systems return to your spare pool first (so you never lose crafted gear), you recover a share of the materials and credits that built the hull, and the docks slot frees up right away (a ship on an active mission cannot be scrapped, and salvaging asks for confirmation). The Salvage Bay, in the Stores program, is where you break down spare systems and salvaged materials: salvage a spare ship system to reclaim a share of the materials that built it (so a full systems bay never blocks you), or break down the rare Damaged Reactor Housing for a tiered roll at rarer materials, with better odds as your Fleet Admiral levels and with the Salvage Operations talent. Salvaging at the bay is a TIMED job and is never instant: choosing Salvage adds an order to the bay's queue, the order reserves its target straight away so nothing else can claim it, and the teardown then runs on a countdown that keeps going while the game is closed. The bay runs one teardown per salvage bay it has, bought on its own Upgrades tab, and anything beyond that waits in the queue (see the Order Queues topic). Salvaged materials can be queued in batches, in which case the units start one at a time as bays free up. You can also choose which item qualities ask for confirmation first, on the bay's Rules tab, so a tier you are happy to scrap goes into the queue without a dialog. Salvaging a whole ship at the Docks is the exception: it is not queued, and it still resolves the moment you confirm.",
  },
  {
    id: "autoSalvage",
    title: "Auto-Salvage",
    body: "Auto-salvage is an opt-in set of rules that queues spare ship systems for you, set up on the Salvage Bay's Rules tab. It is off by default, and even switched on it only ever ADDS ORDERS to the bay's queue, where you can still remove one before it starts, so nothing is destroyed without passing through a queue you can see. Two rules feed it and you can run either or both: a QUALITY rule that queues every spare at a chosen quality tier or below, and a DUPLICATES rule that keeps the best copy of each blueprint in each slot (ranked by item level, then quality, then rarity) and queues the rest. The rules will never touch a system installed on a ship, never destroy a Standard-Issue baseline (those return nothing, so removing one stays a deliberate manual choice), and never queue something that is already queued or already being broken down. The interlock worth knowing about: a quality tier you have checked under Confirm before salvaging can NEVER be auto-salvaged, because the rules will not answer a confirmation on your behalf, and the shipped default asks for confirmation on every tier. So a fresh save that turns auto-salvage on will correctly do nothing until you uncheck a tier there. The rules also leave you room to work: from a queue depth of 2 upward they always keep one queue slot free for your own orders. At the base depth of 1 there is no spare slot to keep, so the rules can use it. Removing their order and queueing your own in its place is enough, because a full queue leaves them nothing to take back, and deepening the queue with the Fleet Logistics (Standing Orders) talents restores the reserved slot for good.",
  },
  {
    id: "fuel",
    title: "Fuel",
