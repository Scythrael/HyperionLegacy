...
// combatGear, queues, craftingLevels, autoSalvage, quartermaster) are welcome and
...
//
// ⚠️ AND ONE OF THOSE SIX STOPPED BEING A GUARANTEE (the 0.13.3.1 follow-up that made
// Standard-Issue baselines auto-salvageable). `baseline` is now CONDITIONAL on the player's
// rules reaching the piece, so the flat "never destroys a Standard-Issue baseline" clause this
// topic used to carry became exactly the kind of falsehood the paragraphs above were written
// about: a player would read the manual and trust a protection the engine had stopped applying.
// The clause is rewritten as the condition it now is, in the same change that made it one.
//
...
export const HELP_TOPICS: HelpTopic[] = [
  ...
  },
  {
    id: "quartermaster",
    title: "Quartermaster",
    body: "The Quartermaster, in Facilities, is the fleet's supply counter. Its Requisition tab issues Standard-Issue patterns free, one row per slot that has a baseline: the four economy slots (Cargo Bay, FTL Drive, Reactor Core, Prospecting Rig) and the combat slots (a weapon, a shield emitter, hull plating, and a drone pod). Take one and it lands in your spare systems, ready to install from a ship's Ship Systems screen. It costs nothing and takes no time, because the point of the counter is that an empty slot can always be filled: a ship missing a weapon, a shield emitter, or hull plating cannot launch a patrol, so a free replacement turns that from a dead end into an errand. What you get is the FLOOR and never more. An economy baseline is stat-neutral (a ship carrying one performs exactly as a bare hull does), and a combat baseline restores the hull's own starting numbers rather than adding to them, so nobody gains power by visiting: you only get back to where a new ship starts. The weapon issued here is the same workhorse gun every hull is built with, not a pick from the weapon roster. Real gains come from researching and crafting your own systems. The counter issues one spare of each pattern at a time; install it and it will issue another, and a spare already queued for salvage does not count against you. The Quartermaster has no level and nothing to upgrade, so there is nothing to found and nothing to buy. Its other two tabs, for buying and selling gear, are marked Coming Soon and do nothing yet.",
  },
  {
    id: "storage",
    title: "Storage",
