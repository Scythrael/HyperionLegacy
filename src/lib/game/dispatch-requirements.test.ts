...
// missionControl at level 1 (ore runs unlocked, Salvage/Forage locked). Every test
// below starts from that and mutates ONLY the field it is isolating.
...
describe("canDispatch — happy path (all gates pass)", () => {
  ...
  it("returns { ok: true } for a Salvage run once EVERY gate is satisfied", () => {
    const state = freshState();
    state.facilities = { ...state.facilities, missionControl: { level: 2 } }; // unlock Salvage
    state.captains[0] = { ...state.captains[0], level: 5 }; // clear the captain-level gate
...
describe("canDispatch — one reason per unmet condition (gate order)", () => {
  ...
  it("locked — the mission's unlockLevel exceeds the missionControl level (checked before requirements)", () => {
    const state = freshState(); // missionControl level 1 -> Salvage (unlockLevel 2) locked
    state.fuel = new Decimal(1_000_000); // rule out fuel gates
    // captain is level 1 (< salvage's captain requirement) too, but `locked` is ordered
    // FIRST, so the reason must be `locked`, not `captainLevel`.
    expect(canDispatch(state, 1, "salvageWreckage")).toEqual({ ok: false, reason: "locked" });
  });

  it("captainLevel — mission unlocked but the captain is below requiresCaptainLevel", () => {
    const state = freshState();
    state.facilities = { ...state.facilities, missionControl: { level: 2 } }; // Salvage unlocked
    state.fuel = new Decimal(1_000_000); // rule out fuel gates
    // captain still level 1, below Salvage's requiresCaptainLevel.
...
describe("dispatchCaptainOnMission — consumes canDispatch (reason exposed + fuel deduction intact)", () => {
  ...
  it("surfaces the `locked` reason (Task-6 unlock gate still fires via canDispatch)", () => {
    const state = freshState();
    state.fuel = new Decimal(1_000_000);
    const { success, reason } = dispatchCaptainOnMission(state, 1, "salvageWreckage");
    expect(success).toBe(false);
