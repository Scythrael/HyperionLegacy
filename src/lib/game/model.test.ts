...
import {
  ...
  SLOT_BASE_PHYSICALS,
  // Combat 1.0 (Unit 1.3/1.4): the combat Standard-Issue minter + seeder + baseline magnitudes.
...
describe("HOMEWORLD_TALENTS, launch set", () => {
  ...
  it("Fleet Logistics is the rich category; Homeland Defense/Citizenry are hub-only stubs", () => {
    ...
    // + Queue1/2/3 (the Crafting 0.13.3 queue-depth TRUNK) = 9,
    // + the five per-facility queue BRANCHES (Infrastructure 0.13.4 Phase 1:
    //   Refinery / Fabricator / SalvageBay / ResearchLab / Shipyard) = 14 total.
    // ⚠️ There are five and not six: the Fuel Depot has no queue and never gets a node.
    expect(branches.filter((b) => b === "fleetLogistics").length).toBe(14);
...
// The combat Standard-Issue baseline generator (Combat 1.0, Unit 1.3 + 1.4). Like
// generateStandardIssue (deterministic, no rng, affix-free) but with NON-ZERO combat magnitudes
// PASSED IN BY THE CALLER per-hull (Unit 1.4 made them behavior-preserving: shield/hull match the
// hull's stats, weapons are the full default loadout) and, for a weapon, a weaponType.
// Economy-neutral (mass 0 / powerDraw 0) so installing it never moves an economy stat.
describe("generateCombatStandardIssue (Combat 1.0, Unit 1.3/1.4)", () => {
  it("mints a well-formed shield emitter baseline: the passed shieldCapacity + shieldRecharge, no weaponType", () => {
    const piece = generateCombatStandardIssue({ slotType: "shieldEmitters", shieldCapacity: 300, shieldRecharge: 10, fittedToShipId: "ship-1", allocateId: () => "equip-1" });
    ...
    // Unit 1.4: the per-hull signature magnitudes are passed IN by the caller (behaviour-preserving),
    // no longer read from a module const. Pure-gear shields (design 5a): a 0-capacity emitter would
    // leave the ship with no shield pool, so the caller passes the hull's real shieldCapacity.
    expect(piece.implicitStats).toEqual({ shieldCapacity: 300, shieldRecharge: 10 });
  ...
  it("mints a well-formed hull plating baseline: the passed hullStrength, no weaponType", () => {
    const piece = generateCombatStandardIssue({ slotType: "hullPlating", hullStrength: 480, fittedToShipId: "ship-1", allocateId: () => "equip-1" });
    expect(piece.slotType).toBe("hullPlating");
    expect(piece.weaponType).toBeUndefined();
    expect(piece.implicitStats).toEqual({ hullStrength: 480 });
    expect(piece.durabilityMax).toBe(SLOT_BASE_PHYSICALS.hullPlating.durability);
  });

  it("throws for a shield/plating baseline missing its per-hull magnitude", () => {
    expect(() => generateCombatStandardIssue({ slotType: "shieldEmitters", fittedToShipId: null, allocateId: () => "e" })).toThrow();
    expect(() => generateCombatStandardIssue({ slotType: "hullPlating", fittedToShipId: null, allocateId: () => "e" })).toThrow();
  });
...
// The combat parallel to seedStandardIssueForShip: one weapon per hardpoint in the default loadout
// plus a shield emitter + hull plating for a combat hull, none for an economy hull. The caller
// encodes "is combat" via an empty vs non-empty signatureWeapons list (Unit 1.4).
describe("seedCombatStandardIssueForShip (Combat 1.0, Unit 1.3/1.4)", () => {
  it("mints one weapon per loadout hardpoint + shield emitter + hull plating, in a fixed order with monotonic ids", () => {
    // The destroyer's full default loadout is [autocannon, plasma] (2 weapons), so 2 + shield + plating = 4.
    const spec = specFor("destroyer");
    const { pieces, nextId } = seedCombatStandardIssueForShip("ship-7", spec, 10);
    expect(pieces).toHaveLength(spec.signatureWeapons.length + 2);
    expect(pieces.every((p) => p.fittedToShipId === "ship-7")).toBe(true);
    expect(pieces.map((p) => p.id)).toEqual(["equip-10", "equip-11", "equip-12", "equip-13"]);
    // Deterministic mint order: every weapon in loadout order, then shieldEmitters, then hullPlating.
    expect(pieces.map((p) => p.slotType)).toEqual(["weapon", "weapon", "shieldEmitters", "hullPlating"]);
    // The weapon baselines carry the hull's full loadout, in order.
    expect(pieces.slice(0, 2).map((p) => p.weaponType)).toEqual(spec.signatureWeapons);
    // The shield + plating carry the behaviour-preserving per-hull magnitudes the caller passed.
    expect(pieces[2].implicitStats).toEqual({ shieldCapacity: spec.shieldCapacity, shieldRecharge: spec.shieldRecharge });
    expect(pieces[3].implicitStats).toEqual({ hullStrength: spec.hullStrength });
    expect(nextId).toBe(14); // advanced past the four minted ids
  });
  ...
  it("uses each combat hull's own full loadout (destroyer/battleship/carrier)", () => {
    for (const hull of ["destroyer", "battleship", "carrier"] as const) {
      const spec = specFor(hull);
      const { pieces } = seedCombatStandardIssueForShip("ship-1", spec, 1);
  ...
  it("installing the combat baseline leaves shipDerivedStats BYTE-IDENTICAL to the bare hull (economy-neutral)", () => {
    ...
    // combat hull with its full install resolves missions IDENTICALLY to the bare hull.
    ...
    const combat = seedCombatStandardIssueForShip("ship-1", specFor("destroyer"), 5).pieces;
