...
function depotState(opts: { commonOre?: number; fuel?: number; fuelStorageLevel?: number }): GameState {
  const s = freshState();
...
describe("processFuelPipelines / economyTick — a batch refines 50 ice -> 100 fuel over durationTicks", () => {
  it("consumes 50 ice at start, deposits 100 fuel on completion (tank space available)", () => {
    const state = depotState({ commonOre: 100, fuel: 0, fuelStorageLevel: 0 });
    ...
    expect(afterOne.inventory.commonOre.toString()).toBe("50"); // 50 consumed at start
    ...
    expect(afterDone.inventory.commonOre.toString()).toBe("0"); // 100 - 50 - 50 (2nd batch started)
...
describe("auto-stop — tank full (fuel >= fuelCap)", () => {
  it("pauses (starts no batch, consumes no ice) when the tank is already at cap", () => {
    ...
    const state = depotState({ commonOre: 500, fuel: FUEL_TANK_BASE_CAP, fuelStorageLevel: 0 });
    const after = economyTick(state, 1, () => 0);
    expect(fuelJobs(after)).toHaveLength(0); // no batch started -- tank full
    expect(after.inventory.commonOre.toString()).toBe("500"); // no ice consumed
  ...
  it("RESUMES the instant tank room reappears (structural auto-resume)", () => {
    ...
    const paused = economyTick(
      depotState({ commonOre: 500, fuel: FUEL_TANK_BASE_CAP, fuelStorageLevel: 0 }),
      1,
      () => 0
    );
    ...
    expect(resumed.inventory.commonOre.toString()).toBe("450"); // 500 - 50
  });
});
...
describe("auto-stop — ice out (Deuterium Ice < batch input); no ice stranded", () => {
  it("pauses (starts no batch) when ice < the batch input, leaving the ice UNTOUCHED (not stranded)", () => {
    ...
    const state = depotState({ commonOre: 49, fuel: 0, fuelStorageLevel: 0 });
    const after = economyTick(state, 1, () => 0);
    expect(fuelJobs(after)).toHaveLength(0); // no batch -- not enough ice
    expect(after.inventory.commonOre.toString()).toBe("49"); // NOT stranded -- untouched
    expect(after.fuel.toString()).toBe("0");
  });

  it("RESUMES when ice is replenished", () => {
    const paused = economyTick(depotState({ commonOre: 49, fuel: 0, fuelStorageLevel: 0 }), 1, () => 0);
    const refuelled: GameState = { ...paused, inventory: { ...paused.inventory, commonOre: new Decimal(60) } };
    const resumed = economyTick(refuelled, 1, () => 0);
    expect(fuelJobs(resumed)).toHaveLength(1); // batch started -- ice arrived
    expect(resumed.inventory.commonOre.toString()).toBe("10"); // 60 - 50
  });
});
...
describe("more pipelines -> more fuel per unit time", () => {
  it("a 2-pipeline depot (level 4) refines TWO batches concurrently: +200 fuel in 10 ticks", () => {
    const state = depotState({ commonOre: 200, fuel: 0, fuelStorageLevel: 4 });
    ...
    expect(afterOne.inventory.commonOre.toString()).toBe("100"); // 200 - 2*50
    ...
    expect(afterDone.inventory.commonOre.toString()).toBe("0"); // 200 - 4*50
  });
  ...
  it("upgraded yield + reduced input (level 6): a batch consumes 35 ice and yields 150 fuel", () => {
    ...
    const state = depotState({ commonOre: 35, fuel: 0, fuelStorageLevel: 6 });
    ...
    expect(afterDone.inventory.commonOre.toString()).toBe("0"); // input 35 consumed
  });
});
...
describe("⚠️ offline == live PARITY (tick(bigSpan) == looping economyTick(_,1))", () => {
  it("ICE-OUT mid-span: fuel, ice, and in-flight batches all match", () => {
    ...
    const make = () => depotState({ commonOre: 175, fuel: 0, fuelStorageLevel: 0 });
    const SPAN = 45;
    ...
    expect(offline.inventory.commonOre.toString()).toBe("25");
  ...
  it("TANK-FULL mid-span: fuel, ice, and in-flight batches all match", () => {
    ...
    const make = () => depotState({ commonOre: 400, fuel: 0, fuelStorageLevel: 0 });
    const SPAN = 70;
    ...
    expect(offline.inventory.commonOre.toString()).toBe("150"); // 400 - 5*50
    expect(fuelJobs(offline)).toHaveLength(0);
  });
});
