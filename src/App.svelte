<script lang="ts">
  ...
  const PATCH_NOTES: { version: string; summary: string }[] = [
    { version: "0.9.0", summary: "Four missions available from the start -- Local Asteroid, Lunar Mine Contract, Salvage Skirmish Wreckage, and Forage Minerals & Flora -- each hauling back its own trio of raw materials, 12 in total to feed future crafting. Salvage and Forage ask for a slightly more experienced captain before they'll launch. New Mission Control facility tracks your available missions and their completion counts; more missions will unlock here as new content arrives. Missions now burn fuel for the round trip, and fuel runs on a proper economy: the new Fuel Depot automatically refines Deuterium Ice into fuel through processing pipelines you can expand (upgrade for more pipelines, more fuel per batch, and less ice per batch). If a captain is short at launch, the shortfall is auto-bought with credits as a backup -- costing a short refuel delay on that trip -- and only stops repeating if you're also out of credits. A fuel gauge in the top bar shows at a glance whether you're producing more fuel than you're burning. Each ship still has its own fuel capacity (how far it can range) and engine efficiency (how little it burns). The raw ores are renamed too -- Deuterium Ice (the fuel feedstock) and Titanium. Every mission now shows its XP earned per tick and its dispatch requirements up front (captain level, cargo space, fuel), with a clear reason spelled out when you can't launch. Also: warehouse fill-tiles are smaller on desktop, and their tooltips now open on a single tap on mobile." },
